...
export default async function about() {
    const filePath = path.join(process.cwd(), 'contents', 'howto.md');
    const fileContents = fs.readFileSync(filePath, 'utf8');
    const { data, content } = matter(fileContents);
    const { title, date } = data; // 記事のタイトル
    const processedContent = await remark().use(remarkBreaks).use(remarkHtml).use(remarkGfm).use(remarkRehype, {
        allowDangerousHtml: true,
    }).use(rehypeRaw).use(rehypeStringify).process(content);
